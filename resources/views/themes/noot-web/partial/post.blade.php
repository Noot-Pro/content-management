<article class="group relative flex flex-col md:flex-row bg-white dark:bg-gray-800 rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-700/60 shadow-sm hover:shadow-lg transition-all duration-300">
    @if($post->image() !== null)
    ...
    <div class="p-5 md:p-6 lg:p-8 flex flex-col justify-between w-full relative">
        <div>
            ...
            <a href="{{ route('post',$post->slug) }}" class="block inline-block">
                <h3 class="text-xl md:text-2xl font-bold text-gray-900 dark:text-white leading-tight group-hover:text-[#E86F44] transition-colors duration-300 line-clamp-2">
                    {!! $post->title !!}
                </h3>
            </a>
            @if($post->description !== null)
                <p class="mt-3 text-gray-600 dark:text-gray-300 line-clamp-2 md:line-clamp-3 text-sm leading-relaxed">
                    {!! $post->description !!}
                </p>
            @endif
        </div>

        <div class="flex items-center justify-end mt-5 md:mt-6 pt-5 md:pt-6 border-t border-gray-100 dark:border-gray-700/50">
