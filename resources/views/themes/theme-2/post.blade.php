<div class="mt-8 lg:mt-12 container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl pb-16">
    ...
    <x-slot name="breadcrumbs">
        <li class="flex items-center">
            <a href="{{ route('posts') }}" class="text-gray-500 hover:text-[var(--primary-color)] transition-colors">{{ __('noot-pro-content-management::site.posts') }}</a>
    ...
    <article class="bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100 mb-16 transition-shadow hover:shadow-md">
        ...
        @else
            <div class="p-8 lg:p-12 pb-0 pt-12 relative overflow-hidden">
                <div class="absolute top-0 right-0 -mr-16 -mt-16 w-64 h-64 bg-[var(--primary-color)] rounded-full mix-blend-multiply filter blur-3xl opacity-5"></div>
        ...
        <div class="p-8 lg:p-12 @if($post->image() !== null) pt-10 @endif relative">
            <div class="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-[var(--primary-color)]/20 via-transparent to-transparent"></div>
            <div class="flex flex-col md:flex-row justify-between gap-8 pb-10 mb-10 border-b border-gray-100 relative z-10">
                @if($post->description)
                    <div class="flex-1 md:ltr:pl-6 md:rtl:pr-6 md:border-l-[3px] md:rtl:border-r-[3px] rtl:border-l-0 border-[var(--primary-color)]">
        ...
        <div class="mt-20 mb-12">
            <div class="flex items-center justify-between mb-10">
                <h2 class="text-2xl md:text-3xl font-bold text-gray-900 flex items-center gap-4">
                    <span class="w-8 h-1.5 bg-[var(--primary-color)] rounded-full inline-block"></span>
                    {{ __('noot-pro-content-management::site.related_posts') }}
                </h2>
                <a href="{{ route('posts') }}" class="hidden sm:inline-flex items-center text-[var(--primary-color)] hover:opacity-80 font-bold transition-colors group">
