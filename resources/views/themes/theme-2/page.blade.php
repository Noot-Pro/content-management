    ...
    <x-slot name="breadcrumbs">
        @if($post->parent !== null)
            <li class="flex items-center">
                <a href="{{ route('page',[$post->parent->slug]) }}" class="text-gray-500 hover:text-[var(--primary-color)] transition-colors capitalize text-sm font-medium" aria-current="page">{{ $post->parent->title }}</a>
    ...
    <article class="bg-white rounded-[2rem] overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100/80 mb-16 transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
        ...
        @else
            <div class="px-8 sm:px-10 lg:px-14 pb-0 pt-14 lg:pt-16 relative overflow-hidden bg-gray-50/30">
                <div class="absolute top-0 right-0 -mr-20 -mt-20 w-72 h-72 bg-[var(--primary-color)] rounded-full mix-blend-multiply filter blur-[80px] opacity-10"></div>
                <div class="absolute bottom-0 left-0 -ml-20 -mb-20 w-72 h-72 bg-[var(--primary-color)] rounded-full mix-blend-multiply filter blur-[80px] opacity-10"></div>
                ...
                <div class="flex flex-wrap items-center gap-3 mb-6 relative z-10">
                    ...
                    <span class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white text-gray-500 text-xs font-bold uppercase tracking-widest border border-gray-200/60 shadow-sm hover:shadow transition-shadow">
                        @svg('heroicon-o-calendar', 'w-4 h-4 text-[var(--primary-color)]')
                        {{ optional($post->published_at)->isoFormat('LL') ?? '' }}
                    </span>
                </div>
                ...
                <div class="flex items-center gap-2 mt-8 mb-4 relative z-10">
                    <div class="w-12 h-1.5 bg-[var(--primary-color)] rounded-full"></div>
                    <div class="w-2 h-1.5 bg-[var(--primary-color)] rounded-full opacity-70"></div>
                </div>
            </div>
        @endif
        ...
        <div class="p-8 sm:p-10 lg:p-14 relative bg-white">
            ...
            @if($post->description)
                <div class="pb-10 mb-10 border-b border-gray-100 relative z-10">
                    <p class="text-gray-500 text-xl md:text-2xl leading-relaxed font-light text-balance rtl:border-r-4 ltr:border-l-4 rtl:pr-6 ltr:pl-6 border-[var(--primary-color)] bg-gradient-to-l from-gray-50/50 to-transparent p-4 rounded-xl rounded-r-none rtl:rounded-l-none rtl:rounded-r-xl">
                        {{ $post->description }}
                    </p>
                </div>
            @endif
            ...
            <div class="prose prose-lg max-w-none relative z-10
                        ...
                        prose-a:text-[var(--primary-color)] prose-a:font-semibold hover:prose-a:opacity-90 prose-a:decoration-[var(--primary-color)] hover:prose-a:decoration-[var(--primary-color)] transition-colors
                        prose-img:rounded-3xl prose-img:shadow-xl prose-img:border prose-img:border-gray-100/50 prose-img:my-10
                        prose-strong:text-gray-900 prose-strong:font-bold
                        prose-blockquote:border-l-4 prose-blockquote:border-[var(--primary-color)] prose-blockquote:bg-gray-50/80 prose-blockquote:px-8 prose-blockquote:py-6 prose-blockquote:rounded-3xl prose-blockquote:rounded-tl-none rtl:prose-blockquote:rounded-tr-none rtl:prose-blockquote:rounded-tl-3xl prose-blockquote:italic prose-blockquote:text-gray-700 prose-blockquote:my-8 prose-blockquote:shadow-sm
                        prose-ul:list-disc prose-ol:list-decimal rtl:prose-ul:pr-8 rtl:prose-ol:pr-8 ltr:prose-ul:pl-8 ltr:prose-ol:pl-8 prose-li:text-gray-600 prose-li:marker:text-[var(--primary-color)] prose-li:mb-3 leading-loose text-[1.1rem]">
                {!! $post->getContent() !!}
            </div>
